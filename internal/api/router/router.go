...
// /api/v1/buem/topology (a topology, multi-building), POST
// /api/v1/buem/building (single building, no topology wrapper), and POST
// /api/v1/buem/validate (pre-flight-checks a /building request without
// calling BuEM). Each name matches its own request shape rather than a
// vague lifecycle verb — an earlier version called the topology endpoint
// /start, which said nothing about what it accepted or returned. The
